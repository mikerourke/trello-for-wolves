...
describe('BRD | Board Resource', function() {
  ...
  describe('BRD-U | Board PUT requests', function() {
    ...
    it('BRD-U-03-T01 | updates the description', function(done) {
      trello
        .boards(boardId)
        .updateDescription('BRD-U-03-T01')
        .then(logResponse)
        .should.eventually.be.fulfilled.notify(done);
    });
    ...
    it('BRD-U-05-T01 | updates the name of the Blue label', function(done) {
      trello
        .boards(boardId)
        .updateLabelNameForColor('blue', 'BRD-U-05-T01')
        .then(logResponse)
        .should.eventually.be.fulfilled.notify(done);
    });
    ...
    it('BRD-U-06-T01 | updates the name of the Green label', function(done) {
      trello
        .boards(boardId)
        .updateLabelNameForColor('green', 'BRD-U-06-T01')
        .then(logResponse)
        .should.eventually.be.fulfilled.notify(done);
    });
    ...
    it('BRD-U-07-T01 | updates the name of the Orange label', function(done) {
      trello
        .boards(boardId)
        .updateLabelNameForColor('orange', 'BRD-U-07-T01')
        .then(logResponse)
        .should.eventually.be.fulfilled.notify(done);
    });
    ...
    it('BRD-U-08-T01 | updates the name of the Purple label', function(done) {
      trello
        .boards(boardId)
        .updateLabelNameForColor('purple', 'BRD-U-08-T01')
        .then(logResponse)
        .should.eventually.be.fulfilled.notify(done);
    });
    ...
    it('BRD-U-09-T01 | updates the name of the Red label', function(done) {
      trello
        .boards(boardId)
        .updateLabelNameForColor('red', 'BRD-U-09-T01')
        .then(logResponse)
        .should.eventually.be.fulfilled.notify(done);
    });
    ...
    it('BRD-U-10-T01 | updates the name of the Yellow label', function(done) {
      trello
        .boards(boardId)
        .updateLabelNameForColor('yellow', 'BRD-U-10-T01')
        .then(logResponse)
        .should.eventually.be.fulfilled.notify(done);
    });
      ...
      trello
        ...
        .then(response => {
          ...
          const newMember = members.find(
            member => member.memberType === 'ghost',
          );
          if (newMember) {
